...
# - 1000-token aliases are optional because contract multiplier differs.
ENABLE_STOCK_ALIASES = os.getenv("ENABLE_STOCK_ALIASES", "true").lower() in {"1", "true", "yes", "on"}
ENABLE_1000_ALIASES = os.getenv("ENABLE_1000_ALIASES", "false").lower() in {"1", "true", "yes", "on"}
...
# Внутри бота используется canonical symbol.
# Для обычных одинаковых тикеров ничего делать не нужно:
# BTCUSDT на Binance и BTC_USDT на MEXC после normalize станут BTCUSDT.
#
# Тут нужны только случаи, где тикер реально разный:
# - нефть / commodities
# - metals
# - STOCK suffix на MEXC
# - 1000TOKEN контракты, если включить вручную

# Manual aliases where exchanges use different names.
# canonical -> {binance raw normalized, mexc raw normalized}
TICKER_ALIASES: dict[str, dict[str, str]] = {
    # =====================
    # OIL / ENERGY
    # =====================
    # Binance BZ = Brent, MEXC UKOIL
    "BZUSDT": {
        "binance": "BZUSDT",
        "mexc": "UKOILUSDT",
    },
    # Binance CL = WTI / US crude, MEXC USOIL
    "CLUSDT": {
        "binance": "CLUSDT",
        "mexc": "USOILUSDT",
    },
    # Binance NATGAS, MEXC NGAS
    "NATGASUSDT": {
        "binance": "NATGASUSDT",
        "mexc": "NGASUSDT",
    },
    ...
    # METALS
    # =====================
    # Silver: Binance XAG, MEXC SILVER
    "XAGUSDT": {
        "binance": "XAGUSDT",
        "mexc": "SILVERUSDT",
    },
    # Gold: user noted MEXC GOLD vs Binance XAUT.
    # In the exported table XAU/XAUT were already exact-common, but this covers
    # cases where MEXC exposes GOLD_USDT.
    "XAUTUSDT": {
        "binance": "XAUTUSDT",
        "mexc": "GOLDUSDT",
    },
    ...
    # =====================
    # STOCKS WITH NON-STOCK MEXC NAME
    # =====================
    # Coinbase sometimes appears as COINBASE on MEXC, but COIN on Binance.
    "COINUSDT": {
        "binance": "COINUSDT",
        "mexc": "COINBASEUSDT",
    },
    ...
    # =====================
    # OPTIONAL 1000-TOKEN ALIASES
    # =====================
    # Disabled by default because contract multiplier differs.
    # Enable with ENABLE_1000_ALIASES=true only if you want these compared.
    "BOBUSDT": {
...
def build_alias_maps() -> tuple[dict[str, str], dict[str, str]]:
    ...
    mexc_to_canonical: dict[str, str] = {}

    for canonical, item in TICKER_ALIASES.items():
        is_1000_alias = (
            item.get("binance", "").startswith(("1000", "1000000", "1M"))
            or item.get("mexc", "").startswith(("1000", "1000000", "1M"))
        )

        if is_1000_alias and not ENABLE_1000_ALIASES:
            continue
        ...
        if b:
            binance_to_canonical[normalize_symbol(b)] = normalize_symbol(canonical)

        if m:
            mexc_to_canonical[normalize_symbol(m)] = normalize_symbol(canonical)

    return binance_to_canonical, mexc_to_canonical


BINANCE_ALIAS_MAP, MEXC_ALIAS_MAP = build_alias_maps()


def normalize_binance_symbol(symbol: str) -> str:
    normalized = normalize_symbol(symbol)
    return BINANCE_ALIAS_MAP.get(normalized, normalized)
...
def normalize_mexc_symbol(symbol: str) -> str:
    ...
    # MEXC stock futures:
    # NVDASTOCK_USDT / NVDA_STOCK_USDT / NVDASTOCKUSDT -> NVDAUSDT
    #
    # Это правило включено по умолчанию, потому что в твоём export почти все
    # акции на MEXC имеют STOCK suffix, а на Binance идут без него.
    if ENABLE_STOCK_ALIASES and normalized.endswith("STOCKUSDT"):
        stock_base = normalized.replace("STOCKUSDT", "USDT")
        return MEXC_ALIAS_MAP.get(normalized, stock_base)

    return MEXC_ALIAS_MAP.get(normalized, normalized)
...
async def fetch_binance(session: aiohttp.ClientSession) -> dict[str, FundingInfo]:
    ...
    for item in premium_data:
        ...
        interval = interval_map.get(raw_symbol, 8)
        ...
        result[symbol] = FundingInfo(
            ...
            interval_source="api" if raw_symbol in interval_map else "default",
        )
...
async def export_exchange_symbols() -> tuple[Path, Path, dict[str, Any]]:
    """
    Exports Binance/MEXC futures symbols to xlsx and txt.
    The xlsx has an Alias template sheet for manual ticker matching.
    """
    headers = {"User-Agent": "funding-interval-bot-symbol-export-v1"}
    connector = make_connector()
    ...
    binance_symbols = sorted(binance)
    mexc_symbols = sorted(mexc)
    common = sorted(set(binance_symbols) & set(mexc_symbols))
    only_binance = sorted(set(binance_symbols) - set(mexc_symbols))
    only_mexc = sorted(set(mexc_symbols) - set(binance_symbols))

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    xlsx_path = Path(f"symbols_export_{timestamp}.xlsx")
    txt_path = Path(f"symbols_export_{timestamp}.txt")
    ...
    try:
        ...
        raise RuntimeError("Для /export_symbols нужно установить openpyxl: pip install openpyxl") from exc
    ...
    ws.append([
        ...
        "MEXC normalized",
        "MEXC raw",
        "MEXC interval cached",
        "MEXC funding",
        "",
        "Manual alias note",
    ])

    max_len = max(len(binance_symbols), len(mexc_symbols))

    for i in range(max_len):
        b_symbol = binance_symbols[i] if i < len(binance_symbols) else ""
        m_symbol = mexc_symbols[i] if i < len(mexc_symbols) else ""

        b = binance.get(b_symbol) if b_symbol else None
        m = mexc.get(m_symbol) if m_symbol else None

        ws.append([
            b_symbol,
            b.exchange_symbol if b else "",
            fmt_interval(b.interval_hours) if b else "",
            fmt_rate(b.rate) if b else "",
            "",
            m_symbol,
            m.exchange_symbol if m else "",
            fmt_interval(m.interval_hours) if m else "",
            fmt_rate(m.rate) if m else "",
            "",
            "",
        ])

    ws_common = wb.create_sheet("Exact common")
    ws_common.append(["Normalized symbol", "Binance raw", "MEXC raw"])
    for symbol in common:
        ws_common.append([
            symbol,
            binance[symbol].exchange_symbol,
            mexc[symbol].exchange_symbol,
        ])

    ws_b = wb.create_sheet("Only Binance")
    ws_b.append(["Binance normalized", "Binance raw", "Interval", "Funding"])
    for symbol in only_binance:
        item = binance[symbol]
        ws_b.append([
            symbol,
            item.exchange_symbol,
            fmt_interval(item.interval_hours),
            fmt_rate(item.rate),
        ])

    ws_m = wb.create_sheet("Only MEXC")
    ws_m.append(["MEXC normalized", "MEXC raw", "Interval cached", "Funding"])
    for symbol in only_mexc:
        item = mexc[symbol]
        ws_m.append([
            symbol,
            item.exchange_symbol,
    ...
    ws_alias = wb.create_sheet("Alias template")
    ws_alias.append([
        "canonical_symbol",
        "binance_symbol",
        "mexc_symbol",
    ...
    ws_alias.append([
        ...
        "1000EXAMPLEUSDT",
        "EXAMPLE_USDT",
        "Заполняй сюда пары, которые по факту один и тот же актив",
    ])

    ws_current_aliases = wb.create_sheet("Current aliases")
    ws_current_aliases.append(["canonical_symbol", "binance_symbol", "mexc_symbol", "note"])
    for canonical, item in sorted(TICKER_ALIASES.items()):
        ws_current_aliases.append([
            canonical,
            item.get("binance", ""),
            item.get("mexc", ""),
            "1000 aliases require ENABLE_1000_ALIASES=true" if item.get("binance", "").startswith(("1000", "1000000")) else "",
        ])
    ws_current_aliases.append(["*STOCKUSDT", "", "MEXC STOCK suffix", "Handled by rule: NVDASTOCKUSDT -> NVDAUSDT"])
    ...
    with txt_path.open("w", encoding="utf-8") as f:
        f.write("BINANCE SYMBOLS\n")
        f.write("================\n")
        for symbol in binance_symbols:
            item = binance[symbol]
            f.write(f"{symbol} | raw={item.exchange_symbol} | interval={fmt_interval(item.interval_hours)} | funding={fmt_rate(item.rate)}\n")
        ...
        f.write("\n\nEXACT COMMON SYMBOLS\n")
        f.write("====================\n")
        for symbol in common:
            f.write(f"{symbol} | Binance={binance[symbol].exchange_symbol} | MEXC={mexc[symbol].exchange_symbol}\n")
        ...
        f.write("=========\n")
        for symbol in only_mexc:
            f.write(f"{symbol} | raw={mexc[symbol].exchange_symbol}\n")

    stats = {
        "binance": len(binance_symbols),
        "mexc": len(mexc_symbols),
        "common": len(common),
        "only_binance": len(only_binance),
        "only_mexc": len(only_mexc),
    }

    return xlsx_path, txt_path, stats


async def export_symbols_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not is_admin(update.effective_user.id):
        return

    msg = await update.message.reply_text("Готовлю export символов Binance/MEXC...")

    try:
        xlsx_path, txt_path, stats = await export_exchange_symbols()
    except Exception as exc:
        logger.exception("Symbol export failed")
        await msg.edit_text(f"❌ Ошибка export_symbols:\n{exc}")
        return
    ...
    await msg.edit_text(
        ...
        f"MEXC: {stats['mexc']}\n"
        f"Exact common: {stats['common']}\n"
        f"Only Binance: {stats['only_binance']}\n"
        f"Only MEXC: {stats['only_mexc']}\n\n"
        "Заполни лист Alias template или пришли файл обратно — сделаем словарь сопоставлений."
    )
    ...
    with xlsx_path.open("rb") as f:
        await update.message.reply_document(
            ...
            caption="Excel export для ручного сопоставления тикеров",
        )
    ...
    with txt_path.open("rb") as f:
        await update.message.reply_document(
            ...
            caption="TXT export списков тикеров",
        )
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        "/export_symbols — выгрузить все тикеры Binance/MEXC в xlsx и txt\n"
        "Aliases: MEXC STOCK suffix, oil и metals aliases включены автоматически\n"
...
def build_app() -> Application:
    ...
    app.add_handler(CommandHandler("export_symbols", export_symbols_cmd))
